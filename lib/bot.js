...
var help_msg = `<b>/search {site} {query}</b> - <i>Search Torrents</i> \
\n <b>/details {site} {link}</b> - <i>Get details of a magnet url</i>\
\n <b>/download {Magnet link}</b> - <i>Start download</i>\
\n <b>/remove {magnet link}</b> - <i>Remove an added torrent</i>\
\n <b>/status {magnet link}</b> - <i>Status of an added torrent</i>\
\n Advanced help down`;

function bot(torrent, bot) {
  bot.onText(/\/start/, async msg => {
    var start_msg = `Hello ${msg.from.first_name}\
     \n\n I can leech torrent and save them to the cloud`;
    var options = {
      reply_markup: JSON.stringify({
        inline_keyboard: [
          [
            { text: "Web Client", url: "https://torr-gdrive.herokuapp.com" },
            { text: "Help", callback_data: "help" }
          ],
          [{ text: "Any Bugs?", url: "https://t.me/phantom2152" }]
        ]
      })
    };
  ...
  bot.on("callback_query", callbackQuery => {
    ...
    if (callBackData == "Help") bot.sendMessage(help_msg, callBackData);
  });
  ...
  bot.onText(/\/help/, async msg => {
    ...
    bot.sendMessage(msg.chat.id, help_msg, adv_help);
  });
